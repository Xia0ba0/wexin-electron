      ...
      <div class="navbar">
        ...
        <div class="navbar-bottom">
          <a
            ...
            class="popper-link setting-link"
            @click.stop="settingPopuped = true;infoPopuped = false;"
            title="更多"
          >
          ...
          <div class="popup-head">
            <div class="name-box text-left">
              <span>
                ...
                <i
                  class="icon"
                  :class="[currentUser.sex === 'female' ? 'icon-girl' :'icon-boy']"
                ></i>
              </span>
      ...
      <div class="contact-box">
        ...
        <div class="contact-container full-height" v-else>
          <div class="recordlist list scrolling full-height" v-show="index == 0">
            <ul>
              <li
                v-for="(user, i) in rangRecordList"
                :key="i"
                :class="{ active: activeIndex === i }"
                @click="openChatBox(user, i)"
              >
                <div class="li-img">
                ...
                <div class="li-text">
                  <span class="name">
                    ...
                    <label class="time">{{user.lastRecordTime | formatDate}}</label>
                  </span>
          ...
          <div class="contactlist list scrolling full-height" v-show="index == 1">
            <ul>
              <li
                v-for="(people, index) in contactlists"
                :key="index"
                @dblclick="pushToRecord(people)"
              >
                <div class="li-img">
                  <img
                    :src="people.image"
                    alt="chat-list-img"
                    :class="people.isLogin ? '' : 'gray'"
                  />
                </div>
                <div class="li-name">
                  <span class="name">
        ...
        <div class="chat-body">
          ...
          <div class="chat-send">
            ...
            <textarea
              ...
              id="chatcontent"
              rows="5"
              :disabled="!chattingUser || !chattingUser.isLogin"
              v-model="chatcontent"
              ref="chatcontent"
              v-focus="textareafocused"
              @keydown="keyDown"
            ></textarea>
...
export default {
  ...
  mounted() {
    ...
    this.$EventBus.$on("onmessage", data => {
      ...
      if (data.type === "message") {
        ...
        if (this.chattingUser && data.user_id !== this.chattingUser._id) {
          // 不是正在聊天的窗口，不默认打开聊天框
  ...
  filters: {
    formatDate(value, format = "YYYY-MM-DD") {
      // 用法 DOM元素过滤 | formatDate('YYYY-MM-DD hh:mm')
  ...
  computed: {
    ...
    rangRecordList() {
      let lists = this.recordlists.sort(
